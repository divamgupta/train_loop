...
def parquets_iter_batched(data_dir , split, start=0, step=1, shuffle=True):
    """
...
def yield_tokens(parquet_data_dir , tokenizer_dir , B, n_tokens, split, tokenizer_threads=4, tokenizer_batch_size=128, device="cuda", shuffle=True):
    ...
    def document_batches():
        while True:
            ...
            for batch in parquets_iter_batched(data_dir=parquet_data_dir, split=split , shuffle=shuffle):
...
class BasicTokensDL(torch.utils.data.Dataset):
    def __init__(self, parquet_data_dir , tokenizer_dir, n_tokens , split="train" , len= 1000000):
